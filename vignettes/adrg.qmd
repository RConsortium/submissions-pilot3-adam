...
6.  We can now Install the {pilot3utils.zip} package.

    -   !!! NEED TO INCLUDE STEPS INSTALLING FROM .ZIP !!!
...
#### Package issues
* `haven::read_xpt()`
Some attributes are dropped after using `haven::write_xpt()` `haven::read_xpt()`, e.g., type, length. 
to do:  further check

* `xportr::xportr_length()` 
`NA_character_` is considered as length 2 - [issue resolved on Dec 14, 2022](https://github.com/tidyverse/haven/issues/699) 
to do: to be incorporated in pilot3

#### Potential improvement
* `metatools::build_from_derived()`
https://github.com/pharmaverse/metatools/issues/46
Not urgent feature but nice to have - especially when `define.xml` is not in good quality

### Knowledge sharing
Summary of similarities and differences between packages, to help user identify the best tool that suits the need. 
Maybe this could go to [Bayer SAS2R catalog](https://bayer-group.github.io/sas2r/) in the future, 
