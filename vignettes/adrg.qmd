...
::: callout-note
In the example file paths for `adam` and `output`, you will need to create these folders first, in their respective locations and set those accordingly in this path list. The reason for creating a new folder (e.g ../adam/datasets-fda) is to not overwrite the current ADaM datasets generated by the sponsor (i.e. ../adam/datasets), upon FDA reviewers' execution of the analysis programs.
...
path <- list(
  ...
  adam = "Enter full file path to where adam data should be saved and 
  sourced here",
...
path <- list(
  sdtm = "~/m5/datasets/rconsortiumpilot3/tabulations/sdtm",
  adam = "~/m5/datasets/rconsortiumpilot3/analysis/adam/datasets-fda",
  output = "~/m5/datasets/rconsortiumpilot3/analysis/output"
  )
```
:::
