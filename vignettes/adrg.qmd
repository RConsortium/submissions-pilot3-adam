...
6.  Install the {pilot3utils} package following the steps below.
    ...
    1.  In R Studio, navigate to your `Packages` window pane.

    2.  Then click on `Install`, where a new window called `Install Packages` will show up.

    3.  In the field `Install from:`, click on the drop-down menu and select `Package Archive File (.zip; .tar.gz)`

    4.  In the field `Package Archive`, click on `Browse...` and navigate to this directory to point to the {pilot3utils} package : `/m5/datasets/rconsortiumpilot3/analysis/adam/programs/pilot3utils_0.0.2.zip`

    5.  In the field `Install to Library:`, you may keep this at its default setting.
